...
document.addEventListener('DOMContentLoaded', () => {
    // ១. ដំណើរការមុខងារទាញទិន្នន័យ
    loadProducts();         
    loadCartFromStorage();  

    // ២. ដំណើរការ Auto Hero Slider
    initHeroSlider();

    // ៣. រៀបចំ Search Logic
    const searchInput = document.getElementById('searchInput');
    if (searchInput) {
        searchInput.addEventListener('input', filterProducts);
    }

    // ៤. រៀបចំ Checkout Logic
...
function initHeroSlider() {
    ...
    // Preload រូបភាពទុកមុន ដើម្បីកុំឱ្យទាក់ពេលប្តូរ
